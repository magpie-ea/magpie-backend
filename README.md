...
5. Run `gigalixir create -n APP_NAME` where `APP_NAME` is the custom name of your magpie-backend instance.

6. Run `gigalixir config:set PROD_COOKIE="COOKIE"`, where `COOKIE` could be generated by launching `iex` in the command line and running `Base.url_encode64(:crypto.strong_rand_bytes(40))`

      Note: If you don't have Elixir installed on your computer, you may choose to use some other random generator for this task, which essentially asks for a random 40-character secret. On Mac and Linux, you may run `openssl rand -base64 40`. Or you may use an online password generator [such as the one offered by LastPass](https://lastpass.com/generatepassword.php).

7. Run  `git push gigalixir master`

8. Run `gigalixir pg:create --free`

9. Add your public ssh key to your account with `gigalixir account:ssh_keys:add "$(cat ~/.ssh/id_rsa.pub)"`.
...
10. Wait for a moment before running `gigalixir ps:migrate` to initialize the database.

11. Don't forget to set the environment variables `AUTH_USERNAME` and `AUTH_PASSWORD`.
